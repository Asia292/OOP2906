...
void HUD::update(float timestep)
{
	...
	//// UPDATES COIN TEXT WITH NUMBER OF REMAINING COINS OR 'EXIT' IF 0 ////
	if (coinCount != 0)
...
//// ADDS TO SCORE BASED ON ITEM TYPE ////
...
//// ADDS THE REMAINING TIME AS A BONUS ONTO SCORE ON WIN ////
int HUD::finalScore()
{
	int Mbonus = min * 60;
	int Sbonus = sec;
